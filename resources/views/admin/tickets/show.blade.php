                ...
                <div class="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        ...
                        <div class="text-xs text-slate-500">
                            ...
                            @if($ticket->order_id) • Order #{{ $ticket->order_id }} @endif
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        ...
                        <form method="POST" action="{{ route('admin.tickets.status', $ticket) }}">
                            @csrf
                            <input type="hidden" name="status" value="{{ $ticket->isOpen() ? 'closed' : 'open' }}">
                            <button class="px-3 py-2 rounded-xl border dark:border-slate-600 hover:bg-primary/10">
                                {{ $ticket->isOpen() ? 'Tutup' : 'Buka' }}
                            </button>
                        </form>
                    </div>
                </div>
                    ...
                    @foreach($ticket->messages as $m)
                        <div @class([
                            ...
                            <div class="mb-1 text-xs text-slate-500">
                                {{ $m->is_admin ? ($m->user->name ?? 'Admin') : ($m->user->name ?? 'User') }}
            ...
            @if($ticket->isOpen())
                <div class="p-6 rounded-2xl bg-white dark:bg-white/5 ring-1 ring-slate-200/60 dark:ring-white/10">
                    <form method="POST" action="{{ route('admin.tickets.reply', $ticket) }}" class="space-y-3">
                        @csrf
                        <div>
                            <label class="block text-sm font-medium">Balasan Admin</label>
